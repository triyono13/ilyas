...
@section('title') Data Agen @endsection
...
@section('content')
    @component('common-components.breadcrumb')
         ...
         @slot('title_li') Agent  @endslot
     @endcomponent
     <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <a href="{{route('agen.create')}}" class="btn btn-success waves-effect waves-light" role="button" aria-pressed="true">Tambah Agen</a><hr>
                    <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                        ...
                        <tbody>
                            <tr>
                            @foreach ($agen as $result => $hasil)
                                <td>{{$result + $agen -> firstitem()}}</td>
                                <td>{{$hasil->nama}}</td>
                                <td>11</td>
                                ...
                                <td style="text-align: center;">
                                    <form action="{{route('agen.destroy', $hasil->id)}}" method="POST">
                                        @csrf
                                        @method('delete')
                                        <a href="{{route('agen.show', $hasil->id)}}" data-toggle="popover" data-placement="top" data-trigger="hover" data-content="Detail Agen" class="btn btn-primary btn-flat btn-sm">
